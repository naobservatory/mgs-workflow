...
- Splits them into groups by assigned taxid at the species level if the LCA taxid assignment is at the species level or below, else use the LCA taxid assignment
- Clusters reads within each group using VSEARCH to identify cluster representatives
...
#### Split hits TSVs by selected taxid (`SPLIT_VIRAL_TSV_BY_SELECTED_TAXID`)

This subworkflow takes in viral hits TSVs from `MARK_VIRAL_DUPLICATES`, each of which is annotated by its read group as assigned by `PREPARE_GROUP_TSVS`. Each hits TSV is joined with the viral taxonomy DB generated by the INDEX workflow, then partitioned based on the species-level taxid corresponding to each read's assigned taxid if the LCA taxid assignment is at the species level or below, else it is partitioned using the LCA taxid assignment. The result is a longer series of hits TSVs, each annotated with a combination of group and selected taxid.
...
D --> E
E --> F[Partition joined TSV by selected taxid]
F --> G[Flatten channel]
...
---
flowchart LR
A("Representative FASTA files <br> labeled by group & selected taxid <br> (CLUSTER_VIRAL_ASSIGNMENTS)") --> B[Extract group labels]
