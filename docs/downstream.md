...
Reads are only compared against reads of the same kind. A pair whose mates aligned to one genome on opposite strands (FR or RF), a pair whose mates aligned on the same strand (FF or RR), a pair split across two genomes, and a read with one mate unaligned each carry a different kind of key, and a read with neither mate aligned has no coordinates and is never a duplicate of anything. A read with one mate unaligned is therefore never grouped with a complete pair; that is deliberate, and differs from `samtools markdup`, on the grounds that a single shared coordinate is weak evidence of duplication.
