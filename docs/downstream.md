...
**Alignment-based marking** identifies duplicate reads on the basis of their assigned genome ID and alignment coordinates, as determined by Bowtie2 in the `RUN` workflow. In order to be considered duplicates, two read pairs must be mapped to the same genome ID by Bowtie2, with terminal alignment coordinates that are within a user-specified distance of each other (default 1 nt) at both ends. This fuzzy matching allows for the identification of duplicate reads in the presence of small read errors, alignment errors or overzealous adapter trimming. A read pair with only one mate aligned is keyed on that mate's start coordinate and the strand it aligned to, so two such reads sharing a coordinate but not a strand are not duplicates.
