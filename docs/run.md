...
[^eukarya]: The default Kraken2 database (PlusPF) includes protozoan and fungal genomes alongside human, so a Eukarya assignment is not by itself a human assignment — read the lower-level taxa for that. This differs from the Standard database used before pipeline version 3.4.0.0, where human was effectively the only eukaryote and the domain-level count could be read as *Homo sapiens*.
