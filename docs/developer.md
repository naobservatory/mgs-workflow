...
- Process conventions (see `modules/local/vsearch/main.nf` for an example of a well-written process that follows these conventions):
    ...
    - All processes should have a label specifying Docker containers to use (e.g. `label "BBTools"`). Containers are then specified in `configs/containers.config`.
    - Any processes that are used only for testing should have the `label "testing"`
    ...
    - If a process has only one output (other than ), `emit` it as `output`Use meaningful emit names
...
- Naming:
    - Use `lower_snake_case` for variable names and channel names
    - Use `UPPER_SNAKE_CASE` for process and workflow names
    - Use `camelCase` for module and subworkflow directory names
    - Use lowercase with hyphen for other files (config and test data) 
...
- Add non-nextflow scripts only when necessary; when possible, use existing bioinformatic tools and shell commands rather than creating custom scripts.
...
- Organization:
    - Python and R scripts for a module go in `results/usr/bin/`
    - Rust source code for a module goes in `src` and the binary (compiled with `cargo build --release`) goes in `results/urs/bin`
...
- Python style: loosely follow PEP 8 conventions
...
Tests themselves are organized in the `tests/` following the same structure as the main code. 
```
tests/
├── modules/local/
│   └── toolName/
...
We also have `.github/workflows/end-to-end.yml` which specifies which tests run automatically on PRs through Github Actions. 
 ...
 Config files for tests are organized as follow:
...
- `configs/index-for-run-test.config` (NOT in the `tests/` directory) is used to run the index workflow (in non-test mode) to generate the index used RUN workflow tests.

Small (uncompressed) files test data files are in `test-data/', large test datasets are in S3:
   - We have some small "toy" test data files (with ~1-10 sequence/record each) in `test-data/toy-data`
   - larger public test datasets in `s3://nao-testing. 
   - Results of workflow runs on the test datasets from S3 are in `test-data/<dataset>-results-<workflow>. 
...
- At a minimum, write a test that the process complete successfully on valid input data (`assert process.success`)
...
`tests/modules/local/fastqc/main.nf.test` is an example of a bare-minimum test for a process, and `tests/modules/local/vsearch/main.nf.test` is an example of really good, comprehensive testing!
...
This is the default test dataset that we use for testing the `RUN` and `DOWNSTREAM` workflows on short-read data. It is a small dataset that contains XXX reads from the [Yang 2020](https://www.sciencedirect.com/science/article/abs/pii/S0048969720358514?via%3Dihub) study. The code to generate this data can be found [here](https://github.com/naobservatory/generate-test-dataset/blob/main/).
...
nf-test test --tag run # Runs all tests with the "run" tag; in our codebase the  end-to-end test of the RUN workflow on short-read data
```
...
- First, make sure the changes are expected/desired:
    ...
    - Then, find the new output files and compare them to previous output files. Make sure the changes are expected based on your code changes
        ...
        - New output files are in `.nf-test/tests/<hash>/output` (hash is shown when test runs, e.g. in the example error message above the test hash begins with `7699da69`.
    - Once you are happy with the changes to the output:
        - Update output files in `test-data` by copying changed output files from the `.nf-test` directory to the appropriate location in `test-data`, un-compressing the files, and committing the changes.
...
To contribute to the pipeline, start by creating a new branch off of `dev`. The branch should start with your name, followed by a short description of the feature you're working on, with hypehns (e.g. `will-post-hoc-integrate`)

>[!CAUTION]
> Do not make pull requests from the `dev` or `main` branches.

Please keep PRs small--a single feature or bugfix. For complex features, split across multiple PRs in small, logical chunks if at all possible. It's OK if a PR lays groundwork for a new feature but doesn't actually implement it.  
...
### Creating PRs and sending then for review
...
6. **Request review** from a maintainer on your changes. Current maintainers are jeffkaufman, willbradshaw, katherine-stansifer, and harmonbhasin. 
    - Make sure to **assign* the PR to the desired reviewer so that they see your PR (put them in the "Assignees" section on Github as well as the "Reviewers" section).
7. To merge, you must **have an approving review** on your final changes and all conversations much be resolved. After merging, please delete your branch! 


### Reviewing PRs

Reviewers should carefully read through all code and test changes and make sure they understand them. Reviewers are not required to run/test code themselves.

Reviewers should make sure:
- The PR solves desired the issue in a logical and performant way.
- The code is clear, wel-structured, and maintainable. 
...
7. Make a PR to merge `staging` into `master`. Send it to a maintainer for review.
    - All changes should already have been 
