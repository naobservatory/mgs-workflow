...
- Organization:
    ...
    - Rust source code for a module goes in `src` and the binary (compiled with `cargo build --release`) goes in `results/usr/bin`
...
nf-test test --tag run # Runs test(s) with the "run" tag; this specifies the is end-to-end test of the RUN workflow on short-read data
```
