...
- `genome_id_fwd`: GenBank ID for viral genome matching mate 1. For single-read data, this column doesn't exist.
- `genome_id_rev`: GenBank ID for viral genome matching mate 2. For single-read data, this column doesn't exist.
- `taxid_fwd`: NCBI taxon ID for taxon matching mate 1. For single-read data, this column doesn't exist.
- `taxid_rev`: NCBI taxon ID for taxon matching mate 2. For single-read data, this column doesn't exist.
- `edit_distance`: Edit distance between read and aligned genome (for paired-end data, mate 1's edit distance)
- `edit_distance_rev`: Edit distance between mate 2 and aligned genome. For single-read data, this column doesn't exist.
...
- `ref_start_unclipped`: Location of the first reference base the read covers with clipped bases counted as if they had aligned, i.e. `ref_start` minus the alignment's leading clip (for paired-end data, mate 1's). NA where the aligner reported no CIGAR.
- `ref_end_unclipped`: Location of the last reference base the read covers, on the same convention (for paired-end data, mate 1's).
- `ref_start_unclipped_rev`, `ref_end_unclipped_rev`: The same two coordinates for mate 2. For single-read data, these columns don't exist.
...
- `length_normalized_score`: Length-normalized alignment score: `best_alignment_score` divided by natural log of `query_len`. (For paired-end data, max of the two mates' scores.)
