...
- Correct the documentation of the paired-end mate slots: an unsuffixed column holds mate 1 and its `_rev` counterpart mate 2, and either mate can align to either strand or be leftmost on the reference. Also describe `prim_align_ref_start` as excluding soft-clipped bases and `prim_align_fragment_length` as Bowtie2's TLEN, which counts them. Documentation, comments and error messages only.
- Delete `post-processing/`, whose only content was the similarity-based duplicate marking tool DOWNSTREAM now runs itself, along with its `nao_dedup` submodule, its `networkx` dependency and its pytest path. `rust-tools/mark_duplicates_similarity` is now the only copy. Also delete `docs/rfc-trunk-based-development.md`.
