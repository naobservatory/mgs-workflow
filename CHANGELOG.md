...
- Make trace files generated by Nextflow for RUN and DOWNSTREAM unique across runs by adding timestamps to the filenames (prevents overwriting when running multiple attempts in the same directory).
- Redefine the EXTRACT_VIRAL_READS_SHORT subworkflow, which changes the output tsv file:
