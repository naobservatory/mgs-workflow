...
- Key alignment duplicate marking on each mate's unclipped 5′ coordinate and strand, as `samtools markdup` does, rather than on the two mates' alignment start coordinates. Fixes two over-merging bugs: fragments shorter than the read no longer collapse onto a shared start coordinate (#948), and molecules occupying one span in opposite orientations are no longer grouped (#993). Duplicate groups, `prim_align_dup_exemplar`, `{group}_duplicate_stats.tsv.gz` and the deduplicated columns of `{group}_clade_counts.tsv.gz` all change. DOWNSTREAM now requires RUN output produced at this version or later, since `mark_duplicates` reads the unclipped coordinate columns.
- Emit unclipped alignment coordinates from RUN: `prim_align_ref_start_unclipped`, `prim_align_ref_end_unclipped` and their `_rev` counterparts give each mate's reference span with soft- and hard-clipped bases counted as if they had aligned, which is the coordinate pair `samtools markdup` keys duplicates on. Nothing consumes them yet. (#1005)
- Emit unclipped alignment coordinates from RUN: `prim_align_ref_start_unclipped`, `prim_align_ref_end_unclipped` and their `_rev` counterparts give each mate's reference span with soft- and hard-clipped bases counted as if they had aligned. The pair gives a read's unclipped 5′ coordinate — the start on the forward strand, the end on the reverse — which is what `samtools markdup` keys duplicates on. Nothing consumes them yet. (#1005)
- Correct the documentation of the paired-end mate slots: an unsuffixed column holds mate 1 and its `_rev` counterpart mate 2, and either mate can align to either strand or be leftmost on the reference. Also describe `prim_align_ref_start` as excluding soft-clipped bases and `prim_align_fragment_length` as Bowtie2's TLEN, which counts them. Documentation, comments and error messages only. (#1004)
