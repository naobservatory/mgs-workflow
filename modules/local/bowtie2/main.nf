...
process BOWTIE2 {
    ...
    input:
        ...
        val(par_string)
    output:
        tuple val(sample), path("${sample}_bowtie2_mapped.sam"), emit: sam
        tuple val(sample), path("${sample}_bowtie2_conc_{1,2}.fastq.gz"), emit: reads_conc
        tuple val(sample), path("${sample}_bowtie2_unconc_{1,2}.fastq.gz"), emit: reads_unconc
    shell:
        '''
        in1=!{reads[0]}
        in2=!{reads[1]}
