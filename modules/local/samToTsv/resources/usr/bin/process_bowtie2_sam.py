...
def get_line(fields_dict):
    ...
    fields = [
        fields_dict["query_name"],
        fields_dict["genome_id_best"],
        fields_dict["genome_id_all"],
        fields_dict["taxid_best"],
        fields_dict["taxid_all"],
        fields_dict["fragment_length"],
        fields_dict["genome_id_fwd"],
        fields_dict["genome_id_rev"],
        fields_dict["taxid_fwd"],
        fields_dict["taxid_rev"],
        fields_dict["fragment_length_fwd"],
        fields_dict["fragment_length_rev"],
        fields_dict["best_alignment_score_fwd"],
        fields_dict["best_alignment_score_rev"],
        fields_dict["next_alignment_score_fwd"],
        fields_dict["next_alignment_score_rev"],
        fields_dict["edit_distance_fwd"],
        fields_dict["edit_distance_rev"],
        fields_dict["ref_start_fwd"],
        fields_dict["ref_start_rev"],
        fields_dict["map_qual_fwd"],
        fields_dict["map_qual_rev"],
        fields_dict["cigar_fwd"],
        fields_dict["cigar_rev"],
        fields_dict["query_len_fwd"],
        fields_dict["query_len_rev"],
        fields_dict["query_seq_fwd"],
        fields_dict["query_seq_rev"],
        fields_dict["query_qual_fwd"],
        fields_dict["query_qual_rev"],
        fields_dict["length_normalized_score_fwd"],
        fields_dict["length_normalized_score_rev"],
...
def line_from_single(read_dict):
    ...
    adj_score = float(read_dict["alignment_score"]) / math.log(
        float(read_dict["query_len"])
    )
    out_dict = {
        "query_name": read_dict["query_name"],
    ...
    if read_dict["is_mate_1"]:
        ...
        out_dict["fragment_length_fwd"], out_dict["fragment_length_rev"] = (
            read_dict["fragment_length"],
            "NA",
        )
        out_dict["best_alignment_score_fwd"], out_dict["best_alignment_score_rev"] = (
            read_dict["alignment_score"],
            "NA",
        )
        out_dict["next_alignment_score_fwd"], out_dict["next_alignment_score_rev"] = (
            read_dict["next_best_alignment"],
            "NA",
        )
        out_dict["edit_distance_fwd"], out_dict["edit_distance_rev"] = (
            read_dict["edit_distance"],
            "NA",
        )
        out_dict["ref_start_fwd"], out_dict["ref_start_rev"] = (
            read_dict["ref_start"],
            "NA",
        )
        ...
        out_dict["query_len_fwd"], out_dict["query_len_rev"] = (
            read_dict["query_len"],
            "NA",
        )
        out_dict["query_seq_fwd"], out_dict["query_seq_rev"] = (
            read_dict["query_seq"],
            "NA",
        )
        out_dict["query_qual_fwd"], out_dict["query_qual_rev"] = (
...
def line_from_pair(dict_1, dict_2):
    ...
    if dict_1["is_mate_1"] and dict_2["is_mate_1"]:
        raise ValueError(
            "Both reads are forward reads: {}".format(dict_1["query_name"])
        )
    if not dict_1["is_mate_1"] and not dict_2["is_mate_1"]:
        raise ValueError(
            "Both reads are reverse reads: {}".format(dict_1["query_name"])
        )
    ...
    try:
        adj_score_fwd = float(fwd_dict["alignment_score"]) / math.log(
            float(fwd_dict["query_len"])
        )
        adj_score_rev = float(rev_dict["alignment_score"]) / math.log(
            float(rev_dict["query_len"])
        )
...
def check_pair_status(line_dict, paired):
    ...
    if pair_status not in ["UU", "UP", "CP", "DP"]:
        raise ValueError(
            f"Invalid pair status: {line_dict['query_name']}, {pair_status}"
        )
...
def write_sam_headers_paired(out_file):
    ...
    headers = [
        "query_name",
        "bowtie2_genome_id_best",
        "bowtie2_genome_id_all",
        "bowtie2_taxid_best",
        "bowtie2_taxid_all",
        "bowtie2_fragment_length",
        "bowtie2_genome_id_fwd",
        "bowtie2_genome_id_rev",
        "bowtie2_taxid_fwd",
        "bowtie2_taxid_rev",
        "bowtie2_fragment_length_fwd",
        "bowtie2_fragment_length_rev",
        "bowtie2_best_alignment_score_fwd",
        "bowtie2_best_alignment_score_rev",
        "bowtie2_next_alignment_score_fwd",
        "bowtie2_next_alignment_score_rev",
        "bowtie2_edit_distance_fwd",
        "bowtie2_edit_distance_rev",
        "bowtie2_ref_start_fwd",
        "bowtie2_ref_start_rev",
        "bowtie2_map_qual_fwd",
        "bowtie2_map_qual_rev",
        "bowtie2_cigar_fwd",
        "bowtie2_cigar_rev",
        "query_len_fwd",
        "query_len_rev",
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Process a sorted Bowtie2 SAM output into a TSV with additional information."
    )
    parser.add_argument(
        "-s",
        "--sam",
    ...
    parser.add_argument(
        ...
        dest="paired",
        action="store_false",
        help="Process SAM file as containing unpaired read alignments (default: False).",
    )
