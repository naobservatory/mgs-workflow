...
process CUTADAPT_MANUAL_ADAPTER {
    ...
    output:
        tuple val(sample), path("${sample}_cutadapt.fastq.gz"), emit: reads
        tuple val(sample), path("${sample}_cutadapt.txt"), emit: log
        tuple val(sample), path("${sample}_cutadapt_in.fastq.gz"), emit: input
    shell:
        '''
        output="!{sample}_cutadapt.fastq.gz"
        log="!{sample}_cutadapt.txt"
        par="-a !{adapter} -j !{task.cpus} -m 20 -e 0.1 --action=trim"
        zcat !{reads} | cutadapt ${par} - 2> ${log} | gzip -c > ${output}
        ln -s !{reads} !{sample}_cutadapt_in.fastq.gz
        '''
}