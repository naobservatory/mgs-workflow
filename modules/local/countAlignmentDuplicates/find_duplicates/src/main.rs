...
struct PositionKey {
    genome_id: String,
    ref_start_fwd: Option<i32>,
    ref_start_rev: Option<i32>,
}
...
impl PartialEq for PositionKey {
    fn eq(&self, other: &Self) -> bool {
        ...
        unsafe { compare_positions(self.ref_start_fwd, other.ref_start_fwd, DEVIATION) } &&
        unsafe { compare_positions(self.ref_start_rev, other.ref_start_rev, DEVIATION) }
    }
}
...
impl Hash for PositionKey {
    fn hash<H: Hasher>(&self, state: &mut H) {
        ...
        unsafe {
            hash_position(self.ref_start_fwd, DEVIATION, state);
            hash_position(self.ref_start_rev, DEVIATION, state);
        }
    }
}
...
fn main() -> std::io::Result<()> {
    ...
    if args.len() != 4 {
        eprintln!("Usage: {} <input_file> <output_file> <deviation>", args[0]);
        process::exit(1);
    }
    ...
    let deviation: i32 = args[3].parse().unwrap_or_else(|_| {
        eprintln!("Error: deviation must be an integer");
        process::exit(1);
    });

    // Check if the deviation value is valid
    if deviation != 0 && deviation != 1 && deviation != 2 {
        eprintln!("Error: deviation must be 0, 1, or 2");
        process::exit(1);
    }
