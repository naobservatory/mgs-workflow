...
from filter_viral_sam import filter_viral_sam, parse_sam_line, calculate_normalized_score, create_unmapped_mate
...
class TestParsesamLine:
    def test_parse_basic_sam_line(self):
        ...
        assert parsed['flag'] == 99
        assert parsed['rname'] == 'chr1'
        assert parsed['pos'] == 100
        assert parsed['alignment_score'] == 45
        assert parsed['pair_status'] == 'CP'
    
    def test_parse_missing_tags(self):
        line = "read1\t99\tchr1\t100\t60\t50M\t=\t200\t150\tACGTACGT\tIIIIIIII"
        parsed = parse_sam_line(line)
        
        assert parsed['alignment_score'] is None
        assert parsed['pair_status'] is None
    
    def test_parse_up_status(self):
        line = "read1\t99\tchr1\t100\t60\t50M\t=\t200\t150\tACGTACGT\tIIIIIIII\tAS:i:30\tYT:Z:UP"
        parsed = parse_sam_line(line)
        
        assert parsed['pair_status'] == 'UP'

class TestCalculateNormalizedScore:
    def test_normal_calculation(self):
        import math
        score = calculate_normalized_score(50, 100)
        expected = 50 / math.log(100)
        assert abs(score - expected) < 1e-10
    
    def test_none_score(self):
        score = calculate_normalized_score(None, 100)
        assert score == 0
    
    def test_zero_length(self):
        score = calculate_normalized_score(50, 0)
        assert score == 0
...
class TestCreateUnmappedMate:
    def test_create_mate_from_read1(self):
        ...
        mate_line = create_unmapped_mate(parsed)
        mate_parsed = parse_sam_line(mate_line)
        
        # Should flip from read1 (64) to read2 (128)
        assert mate_parsed['flag'] & 128  # read2
        assert not (mate_parsed['flag'] & 64)  # not read1
        assert mate_parsed['flag'] & 4  # unmapped
        assert mate_parsed['cigar'] == '*'
    ...
    def test_create_mate_from_read2(self):
        ...
        assert mate_parsed['flag'] & 64  # read1
        assert not (mate_parsed['flag'] & 128)  # not read2
        assert mate_parsed['flag'] & 4  # unmapped
...
contaminant\t99\tchr2\t300\t60\t50M\t=\t400\t150\tACGTACGTACGTACGT\tIIIIIIIIIIIIIIII\tAS:i:45\tYT:Z:CP
contaminant\t147\tchr2\t400\t60\t50M\t=\t300\t-150\tTGCATGCATGCATGCA\tIIIIIIIIIIIIIIII\tAS:i:43\tYT:Z:CP"""
        
        contaminant_content = "contaminant\n"
        
        sam_file = self.create_temp_file(sam_content, '.sam')
        contaminant_file = self.create_temp_file(contaminant_content, '.txt')
        output_file = os.path.join(self.temp_dir, 'output.sam')
        
        filter_viral_sam(sam_file, contaminant_file, output_file, 0.1)
    ...
    def test_score_threshold_pair_logic(self):
        ...
        # read1: score 20/log(16) ≈ 7.2 (below threshold 10)
        # read2: score 35/log(16) ≈ 12.6 (above threshold 10)
...
read2\t147\tchr2\t400\t60\t50M\t=\t300\t-150\tTGCATGCATGCATGCA\tIIIIIIIIIIIIIIII\tAS:i:18\tYT:Z:CP"""
        ...
        filter_viral_sam(sam_file, contaminant_file, output_file, 10.0)
    ...
    def test_up_read_missing_mate_primary(self):
        ...
        filter_viral_sam(sam_file, contaminant_file, output_file, 1.0)
        ...
        read2_parsed = parse_sam_line(read2_line.strip())
...
read1\t403\tchr2\t400\t60\t50M\t=\t300\t-150\tTGCATGCATGCATGCA\tIIIIIIIIIIIIIIII\tAS:i:15\tYT:Z:CP"""
        ...
        # Threshold that allows primary (score ~18) but not secondary chr2 (max score ~16)
        filter_viral_sam(sam_file, contaminant_file, output_file, 17.0)
        ...
        assert len(output_lines) == 2
        primary_flags = [parse_sam_line(line.strip())['flag'] for line in output_lines]
...
read1\t403\tchr3\t600\t60\t50M\t=\t500\t-150\tTGCATGCATGCATGCA\tIIIIIIIIIIIIIIII\tAS:i:20\tYT:Z:CP"""
        ...
        filter_viral_sam(sam_file, contaminant_file, output_file, 15.0)
        ...
        references = [parse_sam_line(line.strip())['rname'] for line in output_lines]
        ...
        flags = [parse_sam_line(line.strip())['flag'] for line in output_lines]
