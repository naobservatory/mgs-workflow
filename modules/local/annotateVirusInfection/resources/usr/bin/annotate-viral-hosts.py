...
def check_infection(virus_taxids: pd.Series,
                    ...
                    exclude_taxids: List[str]) -> pd.DataFrame:
    """
    ...
    follows:
        ...
        - Otherwise, the taxon is marked 0.
    If a taxon is included in exclude_taxids, it and all descendants are
    marked 0 regardless of Virus-Host-DB; this is intended to account
...
def annotate_virus_db_single(virus_db: pd.DataFrame,
    ...
    statuses = check_infection(virus_db["taxid"], host_taxids, virus_tree, 
                               virus_host_mapping, exclude_taxids)
...
def main():
    ...
    parser.add_argument("exclude_taxids", help="Space-delimited list of viral taxids to exclude from host annotation.")
    ...
    output_db = annotate_virus_db(virus_db, host_dict_full, virus_host_mapping, exclude_taxids)
    # Write output
