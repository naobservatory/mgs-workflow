...
process ANNOTATE_VIRUS_INFECTION {
    ...
    input:
        ...
        val(soft_exclude_taxids)
    output:
        path("total-virus-db-annotated.tsv.gz"), emit: db
    shell:
        '''
        annotate-viral-hosts.py !{virus_db} !{host_db} !{infection_db} !{nodes_db} "!{hard_exclude_taxids}" "!{soft_exclude_taxids}" total-virus-db-annotated.tsv.gz
        '''
}
